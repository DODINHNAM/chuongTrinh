...
export class QlProductComponent implements OnInit {
  ...
  showModal(id, action): void {
    ...
    if (action == 'VIEW') {
      ...
      this.listOfData.forEach((item) => {
        if (item.productID == id) {
          ...
          this.productForm.controls.productPromotion.setValue(item.productPromotion);
        }
      });
    }
    if (action == 'EDIT') {
    ...
    if (action == 'INSERT') {
      ...
      this.productForm.controls.productPromotion.setValue(0);
    }
  }
  ...
  handleOk(): void {
    ...
    this.product.productPromotion = this.productForm.controls.productPromotion.value;

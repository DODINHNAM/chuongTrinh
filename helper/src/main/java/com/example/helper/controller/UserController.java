...
import com.example.helper.params.SearchProductParam;
...
public class UserController {
	...
	public ResponseEntity<Response<Product>> searchProducts(SearchProductParam searchproductParam) {
		int page = searchproductParam.getPageIndex() - 1;
		int size = searchproductParam.getPageSize();
		Sort sortable = null;
		if (searchproductParam.getSortField() != null && !searchproductParam.getSortField().equalsIgnoreCase("null")) {
			if (searchproductParam.getSortOrder().equals("ascend")) {
				sortable = Sort.by(searchproductParam.getSortField()).ascending();
			}
			if (searchproductParam.getSortOrder().equals("descend")) {
				sortable = Sort.by(searchproductParam.getSortField()).descending();
			}
		} else {
			sortable = Sort.by("productID").descending();
		}
		Pageable pageable = PageRequest.of(page, size, sortable);
		Page<Product> pageBrandPage = serviceUser.searchProduct(searchproductParam, pageable);
		List<Product> lists = pageBrandPage.toList();
		Long count = (long) pageBrandPage.getTotalElements();
		return new ResponseEntity<Response<Product>>(new Response<Product>(count, lists), HttpStatus.OK);
	}
	...
	public ResponseEntity<Response<Bill>> saveBill(@RequestBody Bill bill) {
		if (bill != null) {
			...
			for (Product product : products) {
				...
				if(product.getProductPromotion()>0) {
					billDetail.setPrice((long) (product.getProductPrice()*(1-(product.getProductPromotion()/100.0))));
				}else {
				...
				product.setProductQuantitySold(product.getProductQuantitySold() + product.getQuanlityBuy());
			...
			emailJob.setContent(
					configRepo.getByName("content").getValue().replace("__name__", bill.getName())
	...
	public ResponseEntity<Response<Contact>> saveContact(@RequestBody Contact contact) {
		if (contact != null) {
			...
			contact.setStatus(Constants.PENDING);

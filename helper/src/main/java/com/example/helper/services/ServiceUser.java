...
public class ServiceUser {
	...
	@SuppressWarnings("serial")
	public Page<Product> searchProduct(SearchProductParam searchProductParam, Pageable pageable) {
		...
		Page<Product> listPage = productRepo.findAll(new Specification<Product>() {
			...
			public Predicate toPredicate(Root<Product> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				query.distinct(true);
				List<Predicate> predicates = new ArrayList<>();
				if (searchProductParam.getKey() != null) {
					predicates.add(
							cb.and(cb.like(cb.upper(root.<String>get("productName")), "%" + searchProductParam.getKey().trim().toUpperCase() + "%")));
			
//					predicates.add(
//							cb.and(cb.like(cb.upper(root.get("brand").<String>get("brandName")), "%" + searchProductParam.getKey().trim().toUpperCase() + "%")));
//				
//					predicates.add(
//							cb.and(cb.like(cb.upper(root.get("category").<String>get("categoryName")), "%" + searchProductParam.getKey().trim().toUpperCase() + "%")));
				}
				return cb.and(predicates.toArray(new Predicate[predicates.size()]));
			}
		}, pageable);
		...
		Page<Review> listPage = reviewRepo.findAll(new Specification<Review>() {
			...
			public Predicate toPredicate(Root<Review> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (reviewParam.getProductId() > 0) {
					predicates.add(cb.and(cb.equal(root.get("product").<String>get("productID"), reviewParam.getProductId())));
				}
